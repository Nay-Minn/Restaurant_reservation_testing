// https://github.com/magnum/realtime-password-validator
...
$(function () {
    ...
    $("#validator-output").realtimePasswordValidator({
        debug: true,
        input1: $("#inputPassword"),
        input2: $("#inputConfirmPassword"),
        validators: [
            {
                regexp: ".{8,}",
...
(function ($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            this.settings.input1.on(
                "input",
                { self: this },
                this.validateEvent
            );
            this.settings.input2.on(
                "input",
                { self: this },
                this.validateEvent
            );
        ...
        validateEvent: function (event) {
            ...
            $(self.settings.validators).each(function (index, validator) {
                let valid = false;
                if (validator.regexp)
                    valid = new RegExp(validator.regexp).test(
                        self.settings.input1.val()
                    );
                if (validator.compare)
                    valid =
                        self.settings.input1.val() ==
                        $(self.settings.input2).val();
                const message = $(
